package codenboxautomationlabNEW.codenboxautomationlabNEW;
...
public class AppTest {
	...
	public void StaticDropdown() throws InterruptedException {
		WebElement InputFielddropdown = (driver.findElement(By.id("dropdown-class-example")));
		Thread.sleep(2000);
		Select Dropdown = new Select(InputFielddropdown);
		List<WebElement> options = Dropdown.getOptions();
	    int randomIndex = rand.nextInt(options.size()-1+1);  
	    Dropdown.selectByIndex(randomIndex);
	    
		//Dropdown.selectByIndex(rand.nextInt(3) + 1);
